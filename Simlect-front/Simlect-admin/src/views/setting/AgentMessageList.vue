          ...
          <Table :columns="supportColumns" :fetch="loadSupportSessions" :dataSource="supportData">
            ...
            <template #slotSupportOp="{ row }">
              <div class="support-op">
                ...
                <el-button v-if="row.status === 'QUEUED'" size="small" type="primary" @click="claimSession(row)">认领</el-button>
                <el-button v-if="row.status === 'ASSIGNED'" size="small" type="success" @click="activateSession(row)">接入</el-button>
                <el-button v-if="isSupportActive(row)" size="small" type="success" @click="resolveSession(row)">解决</el-button>
                <el-button v-if="isSupportActive(row)" size="small" @click="returnAi(row)">转回AI</el-button>
              </div>
            </template>
          </Table>
        </div>
...
const isSupportActive = (row) => ['QUEUED', 'ASSIGNED', 'ACTIVE'].includes(row.status)
...
const supportStatusType = (status) => ({
  QUEUED: 'warning',
  ASSIGNED: 'primary',
  ACTIVE: 'success',
  RESOLVED: 'info',
