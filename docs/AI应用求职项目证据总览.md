...
但它仍不是生产经历的替代品。Search/RAG 已从 45/50 条小集扩展到中文 300 商品/120 查询、WANDS 外部人工标注 judged-pool，以及知识扩充后的 144 条 RAG 检索和 40 条生成；当前最重要的未完成项是生成层 badcase、新规则下的命名重跑、真实模型 Agent runtime/消融、授权真实用户试用、Java 交易包覆盖率、完整 live E2E 与长期性能/成本数据。面试时必须主动说明这些边界。
...
| RAG retrieval v3（当前） | 12 份文档/75 chunk/6 FAQ；48 public + 64 regression + 32 fresh 共 144 条；fresh Recall@1/3/5=0.9583/1/1、MRR@10=0.9792、NDCG@5=0.9846、no-answer/injection=1.0 | 真实 Embedding/Rerank，缓存命中/Provider 失败/fallback 均为 0；检索门禁 `PASSED`，但多数 dev 消融 CI 跨 0 |
| RAG generation v3（当前） | 40/40 执行、0 runtime error；自动与 AI 初审 28 PASS/12 FAIL；11,559 input + 1,446 output token | `FAILED_RETAINED`：任务成功 0.70、concept 0.8111、canonical citation correctness/coverage 0.8667/0.9333；冻结旧口径 injection 0.75。后续安全计分修复尚未新 run 验证 |
...
> AI_Shop 是一个 Spring Cloud Alibaba + FastAPI/LangGraph 的单商户电商系统。我做的重点不是套一个聊天界面，而是让 AI 在真实订单、库存、优惠券和售后状态上工作，并把不确定性限制在业务可接受范围内。读操作通过受控 MCP/内部接口查询权威事实；写操作采用 propose、用户确认、Java 状态校验和业务幂等。RAG 使用 Exact FAQ、BM25/向量多路召回、RRF、Rerank、逐条证据门禁、引用和拒答，工具与知识进入模型前做注入检疫和 PII 脱敏。工程上补了五角色 RBAC、HMAC 管理员断言、AI 数据导出/删除和 `aishop-eval/v1`。评测方面，除 27 条 Commerce 和 18 条安全集外，我用真实 Embedding/Rerank 与本地 Elasticsearch 评了中文 300 商品/120 查询、WANDS 4,960 商品/58 查询，并将知识库扩到 12 份/75 chunk 后评了 144 条 RAG。Fresh Recall@3/5=1.0、MRR@10=0.9792，检索门禁通过；40 条答案生成只通过 28 条，因此完整保留 12 个 badcase。它是 `SYNTHETIC + local-live` 的求职证据，不是生产流量或真实用户效果。
...
3. **RAG/Search 的 Recall/MRR 怎么得到？** 中文 Search 标签来自 300 件结构化商品约束，public/fresh/challenge 分开；WANDS 是 58 条查询的完整人工 judged-pool；RAG v3 是 48 public + 64 regression + 32 一次性 fresh，标签来自 12 份已发布知识和 canonical fact catalog。Provider 冷调用后锁定候选，参数消融零 Provider 重放；Search、RAG、合成标签和外部人工标签分别报告。
...
9. **性能和成本结论是什么？** 中文 fresh 的 Embedding/Rerank 本地 P95 为 1360.12/1572.69ms；WANDS 为 1867.62/2577.44ms。v3 检索 fresh 各阶段 P95 合计约 780.80ms；40 条生成端到端 P50/P95 为 3176.33/5031.04ms，TTFT P50/P95 为 2880.76/4496.86ms，共 13,005 token。小样本 P99 不作 SLO；缺可信人民币单价与检索账单，只能写 `UNPRICED`。
10. **最大的工程短板？** 生成 v3 只有 28/40，通过率 0.70，concept 0.8111、canonical citation correctness 0.8667，存在检疫误报、等价标签不足、短答和误拒答；修复后尚未用新 run 验证。Java 交易包行覆盖约 13%，Agent 在线模型、live E2E 和真实用户样本仍缺。
...
本轮 2026-08-13 回归为 Python `945 passed / 7 skipped / 0 failed`（7 条均为显式要求真实 MySQL 8 的迁移用例）、Ruff 通过、bootstrap `7/7`；Java common `59/59`、Search `21/21`，Reactor build 成功。上一轮 Testcontainers `MiddlewareIT 7/7` 与 `TransactionPersistenceIT 3/3` 继续作为 E2 证据，本轮未重复执行。结果目录不入库，CI 上传 artifact；本轮未执行 `--accept-baseline`。
