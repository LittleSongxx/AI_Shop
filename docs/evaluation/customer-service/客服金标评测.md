# AI 客服金标评测（v1）

> 模型报告状态：`PROVISIONAL_NOT_HUMAN_GOLD`；人工复核包状态：`PENDING_ADJUDICATION`；`releaseGateEligible=false`。两位标注者已完成盲标，但 25 条案件仍需 lead reviewer 仲裁，当前不能称为人工准确率。
> 本报告只测客服理解/转接的核心质量证据；Agent `pass^k`、工具契约和终态门禁不计入下表。

数据集：`60` 条；SHA-256：`7c0b874ec9ec72657ec4726bb8cbfc837a22dee87b3c2e2507cddbb003ec85dd`；模式：`rule`。

## 人工金标闭环

仓库冻结的 60 条 draft 已由两位标注者独立完成；sealed 文件、一致性报告和仲裁模板已归档到 `AI_Shop-backend/AI_Shop-agent/evaluation-evidence/benchmarks/customer-service/customer-service-review-v1-pending-adjudication/`，尚未生成 `HUMAN_VERIFIED` 数据集。

```bash
conda activate shop
cd AI_Shop-backend/AI_Shop-agent
python -m evaluation.cli customer-service-review export --annotator reviewer-a --output /tmp/reviewer-a.open.jsonl
python -m evaluation.cli customer-service-review export --annotator reviewer-b --output /tmp/reviewer-b.open.jsonl
# 两位标注者独立填写 labels 后分别封存
python -m evaluation.cli customer-service-review seal --review /tmp/reviewer-a.open.jsonl --output /tmp/reviewer-a.sealed.jsonl
python -m evaluation.cli customer-service-review seal --review /tmp/reviewer-b.open.jsonl --output /tmp/reviewer-b.sealed.jsonl
python -m evaluation.cli customer-service-review compare --review-a /tmp/reviewer-a.sealed.jsonl --review-b /tmp/reviewer-b.sealed.jsonl --output /tmp/reviewer-agreement.evidence.json --markdown-output /tmp/reviewer-agreement.md
python -m evaluation.cli customer-service-review merge --review-a /tmp/reviewer-a.sealed.jsonl --review-b /tmp/reviewer-b.sealed.jsonl --output-dataset /tmp/customer-service-human-v1.jsonl --evidence /tmp/customer-service-human-v1.evidence.json
# 如 validate/merge 报告 reviewer disagreement，再追加 --adjudication /tmp/adjudication.jsonl
```

流程为 `OPEN -> SEALED -> HUMAN_VERIFIED`；sheet 带源数据/内容 SHA-256，禁止写入 `expected`、模型预测或隐藏字段，冲突必须逐 case 仲裁。人工完成前不能把下表称为人工准确率，也不能把人工结果自动纳入 release gate。

盲标表只保留案件 ID 和用户原话；`sliceTags`、`difficulty` 等由数据集派生的上下文不会导出，避免标注者从切片名称反推答案。填写完成后只提交 open sheet，由 CLI 负责 seal、哈希和合并。

## 当前人工一致性证据（仲裁前）

| 维度 | 结果 | 解释 |
|---|---:|---|
| 案件级完全一致 | `35/60 = 58.33%` | 25 条进入仲裁，不能直接合并 |
| `intent` 一致 / Cohen κ | `57/60 = 95.00%` / `0.946` | 意图 taxonomy 基本稳定 |
| `riskLevel` 一致 / Cohen κ | `56/60 = 93.33%` / `0.890` | 物流/确认收货边界需统一 |
| `shouldHandoff` 一致 / Cohen κ | `60/60 = 100%` / `1.000` | 转人工标签最稳定 |
| `handoffSeverity` 一致 / Cohen κ | `56/60 = 93.33%` / `0.826` | 资金异常的紧急级别需仲裁 |
| raw `slots` 完全一致 / Cohen κ | `45/60 = 75.00%` / `0.662` | 主要是 schema 命名差异，不等同模型 Slot F1 |
| 共同出现槽位的值一致 | `24/26 = 92.31%` | `amount` 格式化是主要差异之一 |

冲突 case：`001, 002, 003, 007, 008, 014, 025, 029, 030, 033, 034, 037, 039, 041, 042, 043, 044, 045, 046, 047, 049, 055, 056, 057, 059`。
其中 `001/002/029/033/034/039/041/042/043/044/045/055/059` 主要是 `budget/brand/productCategory/feature` 与生产侧 `amount/productName` 的槽位规范差异；`049/056/057` 是真实意图边界，必须由 lead reviewer 写理由。

## Draft 规则回放指标（不可当作人工结果）

| 指标 | 值 | 分子/分母 | 95% CI | badcase |
|---|---:|---:|---|---:|
| `intentMacroF1` | 1.0 | 20.0/20 | [0.7, 0.95] | 0 |
| `highRiskIntentRecall` | 1.0 | 10/10 | [0.722467, 1.0] | 0 |
| `slotEntitySpanF1` | 1.0 | 344/344 | [1.0, 1.0] | 0 |
| `slotExactMatch` | 1.0 | 34/34 | [0.898485, 1.0] | 0 |
| `handoffRecall` | 1.0 | 14/14 | [0.784689, 1.0] | 0 |
| `criticalHandoffMissRate` | 0.0 | 0/10 | [0.0, 0.277533] | 0 |

## 优化前后诊断（不是 A/B 或人工真值）

优化前 provisional：32 条，Intent Macro-F1 `0.849524`、高风险 Recall `0.333333`、slot EM `0.857143`、handoff Recall `0.8`；历史 badcase：cs-gold-v1-001, cs-gold-v1-002, cs-gold-v1-003, cs-gold-v1-011, cs-gold-v1-022, cs-gold-v1-026, cs-gold-v1-031, cs-gold-v1-032。
当前扩展到 60 条并修复规则后，点估计通过参考门槛；以下数值只是对 assistant-draft 标签的规则回放，不能写成独立人工准确率，也不代表线上客服成功率。
扩展切片：acknowledgement=1; after-sales=3; attribute-question=2; brand=1; budget-constraint=1; chat=1; comparison=1; compatibility=1; complaint=1; consult-search-boundary=2; critical-handoff=5; currency-slot=1; duplicate-charge=1; exchange=1; handoff=1; invoice=1; low-risk-chat=2; missing-item=1; missing-order=1; negation=1; negative-constraint=1; payment=1; payment-policy=1; payment-risk=3; policy-status-boundary=1; privacy=2; product-consult=3; product-search=3; refund=1; repeated-unresolved=1; risk-boundary=1; short-utterance=1; slot-order-id=1; slot-product-name=1; timing-question=1; unauthorized-payment=1; wrong-item=1

## Intent 明细

| Intent | support | Precision | Recall | F1 | badcase |
|---|---:|---:|---:|---:|---:|
| `ADDRESS_CHANGE` | 1 | 1.0 | 1.0 | 1.0 | 无 |
| `AFTERSALES_UNKNOWN` | 1 | 1.0 | 1.0 | 1.0 | 无 |
| `CANCEL_ORDER` | 2 | 1.0 | 1.0 | 1.0 | 无 |
| `CHAT` | 4 | 1.0 | 1.0 | 1.0 | 无 |
| `COMPLAINT` | 3 | 1.0 | 1.0 | 1.0 | 无 |
| `CONFIRM_RECEIPT` | 1 | 1.0 | 1.0 | 1.0 | 无 |
| `DAMAGED_OR_WRONG_ITEM` | 7 | 1.0 | 1.0 | 1.0 | 无 |
| `HUMAN_REQUEST` | 6 | 1.0 | 1.0 | 1.0 | 无 |
| `INVOICE` | 2 | 1.0 | 1.0 | 1.0 | 无 |
| `PAYMENT_ISSUE` | 7 | 1.0 | 1.0 | 1.0 | 无 |
| `PRODUCT_CONSULT` | 5 | 1.0 | 1.0 | 1.0 | 无 |
| `PRODUCT_REVIEW` | 1 | 1.0 | 1.0 | 1.0 | 无 |
| `PRODUCT_SEARCH` | 7 | 1.0 | 1.0 | 1.0 | 无 |
| `QUERY_COUPON` | 1 | 1.0 | 1.0 | 1.0 | 无 |
| `QUERY_FULFILLMENT` | 1 | 1.0 | 1.0 | 1.0 | 无 |
| `QUERY_LOGISTICS` | 2 | 1.0 | 1.0 | 1.0 | 无 |
| `QUERY_ORDER` | 3 | 1.0 | 1.0 | 1.0 | 无 |
| `RECOMMENT` | 1 | 1.0 | 1.0 | 1.0 | 无 |
| `REFUND` | 3 | 1.0 | 1.0 | 1.0 | 无 |
| `REFUND_STATUS` | 2 | 1.0 | 1.0 | 1.0 | 无 |

## Badcase（逐指标）

## 口径与限制

- 人工金标冻结流程：两名标注者已盲标 intent/risk/转人工/严重度/slot；当前状态为 `PENDING_ADJUDICATION`，25 条冲突未仲裁，未产生人工准确率。
- 高风险 Recall 的正类是独立标签 `riskLevel=HIGH`，不是模型自报风险；严重漏转人工只统计 `handoffSeverity=CRITICAL`。
- slot Entity/Span F1 使用 NFKC 后的字符 span；`slotExactMatch` 只在存在 gold slot 的请求上计分，空 slot 不抬高结果。
- `HANDOFF_SUGGESTED` 不算即时转人工成功；远程结果未知、Provider 失败和人工校准不在本基线中伪造。
- 独立人工复核完成后，必须冻结标签版本、重新运行并保留本 provisional 包，不得覆盖历史结果。
