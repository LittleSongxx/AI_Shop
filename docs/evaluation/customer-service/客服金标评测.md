...
- 修复后的 `customer-service-http-v13-20260824` 已完成一次真实 HTTP observation，但 `answerQuality.status=PENDING_HUMAN_REVIEW`；两份独立开放盲审表位于 `evaluation/datasets/customer_service/answer-review-v13/`。旧答案 labels 绑定 source run 和答案 SHA-256，不能迁移到新代码结果。
