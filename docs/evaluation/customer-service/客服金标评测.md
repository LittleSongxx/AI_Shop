...
| `slotEntitySpanF1` | 0.907652 | 688/758 | [0.888889, 0.926454] | 15 |
...
2026-08-23 统计口径修正：Slot F1 现以标准 `2TP/(2TP+FP+FN)=688/758` 展示，bootstrap 每次抽样重新累加
TP/FP/FN 后计算 micro F1；Intent 以 `intent×risk×handoff` 分层并每次重算 Macro-F1。人工标签包保持不可变，旧包内的原始评测器投影不覆盖；
当前口径由项目 manifest 中的本文档 JSON 与 HTTP 证据包哈希追踪。

## 生产槽位对齐诊断

以下投影只覆盖当前生产抽取器已实现的 `orderId/orderItemId/productId/productName/amount`，不替换上面的完整人工 schema 主指标。
- `canonicalSlotEntitySpanF1`：0.992785 （688/693，95% CI [0.98727, 0.997041]，badcase `cs-gold-v1-009, cs-gold-v1-020, cs-gold-v1-055, cs-gold-v1-058`）。
...
同一 60 条 gold 已经过正式 HTTP Agent 路径：执行 `60/60`，HTTP Intent Macro-F1 `0.955299`，handoff `TP=14/TN=46/FP=0/FN=0`，
引用结构违规 `0`。HTTP Episode 槽位会脱敏，所以 HTTP Slot F1/EM 为 `UNAVAILABLE`，本报告的 slot 结果仍只属于规则预路由。
答案正确性、引用语义支持、转人工适当性和 unsafe-answer rate 需两份独立人工盲审，当前为 `PENDING_HUMAN_REVIEW`。
证据包：[customer-service-http-v1-20260823](../../../AI_Shop-backend/AI_Shop-agent/evaluation-evidence/benchmarks/customer-service/customer-service-http-v1-20260823/)。
