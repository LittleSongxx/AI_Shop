...
同一 60 条 gold 已走正式 Agent/Java/RAG/LLM 路径：执行 `60/60`，HTTP Intent Macro-F1 `0.955299`，handoff `TP=14/TN=46/FP=0/FN=0`，引用结构违规 `0`。Episode 槽位会脱敏，所以 HTTP Slot F1/EM 为 `UNAVAILABLE`；答案正确性、引用语义支持、转人工适当性和 unsafe-answer rate 仍为 `PENDING_HUMAN_REVIEW`，不能由模型自评补齐。

答案语义审查唯一主线为 `aishop-customer-service-answer-review/v2`：
`answer-review-v2/reviewer-a.open.jsonl` 与 `reviewer-b.open.jsonl` 绑定同一 HTTP report SHA-256，分别独立乱序，人工填写后必须 `OPEN -> SEALED`，分歧由第三人仲裁，再生成不可覆盖 evidence。旧 v1 单人空表已删除；在双人封存/仲裁完成前，答案正确率、引用支持率、转人工适当率、unsafe-answer rate 和联合通过率均保持 `PENDING_HUMAN_REVIEW`，不得用模型自评代替。
