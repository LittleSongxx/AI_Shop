...
复核质量：60 条中完全一致 `35/60`；intent 一致 `57/60`、Cohen κ `0.946`；risk 一致 `56/60`、κ `0.890`；handoff `60/60`；lead reviewer 仲裁 `25` 条。仲裁文件、两份 sealed 原件和哈希均在证据包内。

## 核心指标

| 指标 | 值 | 分子/分母 | 95% CI | badcase |
|---|---:|---:|---|---:|
| `intentMacroF1` | 0.955299 | 19.105978/20 | [0.642094, 0.926190] | 3 |
...
`intentMacroF1` 的区间是冻结包生成的 case-bootstrap 诊断值。由于观测到的 20 个 intent 标签稀疏，percentile 区间 `[0.642094, 0.926190]` 可能不包含点估计 `0.955299`；这不是分子分母错误，但不能作为总体泛化 CI。后续扩充数据时按 intent 分层 bootstrap，生成新版本并保留本包。
...
**可陈述边界**：这是一份“60 条人工复核离线理解基线”，不是完整 HTTP Agent/LLM 端到端结果。Intent Macro-F1、风险 Recall、slot
F1/EM 和 handoff Recall 可写入简历，但必须同时写样本量、`releaseGateEligible=false` 和 badcase；规则预路由通过率不能替代人工答案质量、
线上客服成功率或 CSAT/FCR。
...
- 当前可用于秋招面试的表述是“60 条人工复核离线理解基线 + 逐指标 badcase”；不能表述为线上客服成功率、CSAT/FCR 或 release gate 结果。优先回归 `011/044/057` 意图边界和 `055/058` canonical 槽位/金额归一化，再决定是否把扩展槽位纳入生产 schema。
