...
> 历史口径提示：本文件只维护 `aishop_convo_v1` 的历史冻结事实。当前项目级结果请看 [`docs/AI应用求职项目证据总览.md`](../../../docs/AI应用求职项目证据总览.md)；`134/134` 不再作为 README 或求职材料中的当前端到端成绩。
...
## 十、2026-08-12 治理层改造后的核验

本轮（课程项目借鉴实施）改动了运行时代码：Observation 层通道检疫（污染内容替换为
占位符）、RAG 片段逐条检疫、Prompt 注册表重构（fragment 注册 + 来源可观测）、
per-request 成本摘要（worker/图入口 reset）、工具观察与记录条件扩展。以上均未改变
冻结会话的意图路由与订单解析语义，重跑 `run_convo_eval.py`：

- 134/134 通过（passRate 1.0），`failedIds` 为空，门禁与 lock 双向一致。
- 无需重建 lock：数据集 SHA-256 与 runner 版本均未变化，基线行保持 134（1.0）。
- 本批 Python 全量测试 838 passed / 7 skipped / 0 失败。`_require_internal_token`
  → `_require_admin` 迁移后原守卫测试同步翻转断言；新增回归测试：exact-FAQ
  快路径污染检疫（H1）、全剔场景 RAG_SEARCH_TOTAL 按 miss 计数（M2）、
  agentic 污染路径检疫痕迹保留（L5）。
