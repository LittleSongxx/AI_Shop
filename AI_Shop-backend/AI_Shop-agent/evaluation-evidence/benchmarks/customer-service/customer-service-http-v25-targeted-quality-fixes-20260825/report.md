...
> `TARGETED_REGRESSION_OBSERVATION`；答案质量仍待独立人工盲审，不进入 release gate。

Run：`customer-service-http-v25-targeted-quality-fixes-20260825`；样本：`10`；数据 SHA-256：`112dfd6ba7546b7cbad317597d944e3ab4dc02627d4ca6018733031d8eddc527`。

| 指标 | 数值 | 分子/分母 | badcase |
|---|---:|---:|---|
| HTTP Intent Macro-F1 | 1.0 | 7.0/7 | - |
| HTTP High-risk Recall | None | 0/0 | - |
| HTTP Handoff Recall | None | 0/0 | - |
| 规则 Slot micro F1 | 0.993151 | 290/292 | cs-gold-v1-009, cs-gold-v1-020 |
| 规则 Slot EM | 0.8 | 8/10 | cs-gold-v1-009, cs-gold-v1-020 |

- HTTP 执行：`10/10`；转人工混淆矩阵：`{'truePositive': 0, 'falsePositive': 0, 'falseNegative': 0, 'trueNegative': 10}`。
- 引用结构无效：`0`，case：`无`；语义支持仍由人工评分。
- 本地全链路延迟 P50/P95/P99：`932.267/1329.563/1514.914 ms`，不是生产 SLO。
...
- 定向安全行为契约：状态 `PARTIAL_NOT_EXECUTED`；已执行/总数 `2/11`；违规 `0`，badcase：`无`。该诊断不等价于人工答案正确率。
