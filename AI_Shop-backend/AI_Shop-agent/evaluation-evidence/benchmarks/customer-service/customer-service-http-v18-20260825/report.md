...
Run：`customer-service-http-v18-20260825`；样本：`60`；数据 SHA-256：`112dfd6ba7546b7cbad317597d944e3ab4dc02627d4ca6018733031d8eddc527`。
...
- 本地全链路延迟 P50/P95/P99：`1012.651/1440.236/7436.927 ms`，不是生产 SLO。
- Usage：input/output token `15278/578`，Provider calls `5`，费用状态 `UNPRICED`，costCny `None`。未知费用不记为 0。
- 运行质量诊断（非人工真值）：Verifier observed/pass `46/42`；安全降级 `4`；澄清生效 `9`；硬约束违规 `0`，badcase：`无`。
- 定向安全行为契约：状态 `SATISFIED`；已执行/总数 `11/11`；违规 `0`，badcase：`无`。该诊断不等价于人工答案正确率。
