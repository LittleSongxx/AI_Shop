...
Run：`customer-service-http-v54-full-badcase-fixes-handoff-evidence-20260827`；样本：`120`；数据 SHA-256：`02a6dacc6a2aadb88c6dfb60bf7a74e2f083fcba0f9a6e82fef38c4dfa82caf3`。
...
- 本地全链路延迟 P50/P95/P99：`611.424/2079.506/7921.439 ms`，不是生产 SLO。
- Usage：input/output token `36849/1821`，Provider calls `13`，费用状态 `UNPRICED`，costCny `None`。未知费用不记为 0。
- 运行质量诊断（非人工真值）：Verifier observed/pass `86/82`；安全降级 `4`；澄清生效 `18`；硬约束违规 `0`，badcase：`无`。
- 定向安全行为契约：状态 `SATISFIED`；已执行/总数 `23/23`；违规 `0`，badcase：`无`。该诊断不等价于人工答案正确率。
