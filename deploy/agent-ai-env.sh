...
load_agent_ai_env() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    key="${BASH_REMATCH[1]}"
    case "$key" in
      LLM_API_KEY|LLM_BASE_URL|LLM_MODEL|EMBEDDING_API_KEY|EMBEDDING_BASE_URL|EMBEDDING_PATH|EMBEDDING_MODEL|EMBEDDING_DIMENSIONS|SPRING_AI_MODEL_EMBEDDING|VECTOR_INDEX|VECTOR_FIELD|VECTOR_INDEX_SCHEMA_VERSION|DASHSCOPE_API_KEY|VLM_ENABLED|VLM_API_KEY|VLM_BASE_URL|VLM_MODEL|VLM_IMAGE_MAX_TOKENS|VLM_MAX_TOKENS|VLM_CONNECT_TIMEOUT_SECONDS|VLM_TIMEOUT|VLM_TIMEOUT_SECONDS)
        ;;
      *)
        continue
        ;;
