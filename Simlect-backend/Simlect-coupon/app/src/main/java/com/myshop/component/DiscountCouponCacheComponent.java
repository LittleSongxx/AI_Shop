...
import com.myshop.config.CouponCacheExecutorConfig;
import com.myshop.constants.Constants;
import com.myshop.entity.dto.CouponLogicalCacheEntry;
import com.myshop.entity.enums.RushingStatusEnum;
...
public class DiscountCouponCacheComponent {
    ...
    public PaginationResultVO<DiscountCoupon> getPlazaList(
        ...
        if (!StringTools.isEmpty(raw)) {
            try {
                CouponLogicalCacheEntry entry = JSON.parseObject(raw, CouponLogicalCacheEntry.class);
                if (entry != null && !StringTools.isEmpty(entry.getPayload())) {
                    PaginationResultVO<DiscountCoupon> cached =
                            JSON.parseObject(entry.getPayload(), PLAZA_LIST_TYPE);
                    if (cached != null) {
    ...
    public DiscountCoupon getDetail(String couponId, Supplier<DiscountCoupon> dbLoader) {
        ...
        if (!StringTools.isEmpty(raw)) {
            try {
                CouponLogicalCacheEntry entry = JSON.parseObject(raw, CouponLogicalCacheEntry.class);
                if (entry != null && !StringTools.isEmpty(entry.getPayload())) {
                    DiscountCoupon cached = JSON.parseObject(entry.getPayload(), DiscountCoupon.class);
                    if (cached != null) {
                        if (!isLogicallyExpired(entry)) {
                            return cached;
                        }
    ...
    private void writeDetailCache(String cacheKey, DiscountCoupon coupon) {
        ...
        writeLogicalEntry(cacheKey, JSON.toJSONString(coupon));
    }
