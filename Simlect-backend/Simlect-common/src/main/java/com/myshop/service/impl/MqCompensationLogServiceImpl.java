...
import com.myshop.compensation.StockBatchCompensatePort;
import com.myshop.compensation.UserCouponStatusCompensatePort;
import com.myshop.component.MqCompensationStore;
import com.myshop.component.MqIdempotencyGuard;
...
import com.myshop.entity.enums.MqCompensationLogStatusEnum;
...
public class MqCompensationLogServiceImpl implements MqCompensationLogService {
    ...
    public void saveFromFailure(MqCompensationRecord record) {
        ...
        logRow.setPayloadJson(JSON.toJSONString(record.getPayload()));
    ...
    private void replayRemoteCompensate(MqCompensationLog existing) {
        ...
        try {
            ...
            if (InternalApiHeaders.REMOTE_STOCK_CHANGE_BATCH.equals(routingKey)) {
                List<ProductItem> items = JSON.parseArray(existing.getPayloadJson(), ProductItem.class);
                ...
                JSONObject payload = JSON.parseObject(existing.getPayloadJson());
                ...
                couponPort.changeUserCouponStatus(
                        payload.getString("userCouponId"),
                        payload.getString("userId"),
                        payload.getInteger("fromStatus"),
                        payload.getInteger("toStatus"),
                        null);
            } else {
    ...
    private void replaySendFailure(MqCompensationLog existing) {
        ...
        try {
            Object payload = JSON.parse(existing.getPayloadJson());
